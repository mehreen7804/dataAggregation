...
var combineKeys = [];
var groupByParams = ["user", "category"];
...
function groupBy(data, keys) { // `data` is an array of objects, `keys` is the array of keys (or property accessor) to group by
  ...
  return data.reduce((storage, item) => {
    ...
    //   if (item[key].toLowerCase() === "misc") {
    //     values[key] = "other";
    //   } else if (item[key].toLowerCase() === "drinks") {
    //     values[key] = "drink";
    //   } else {
        values[key] = item[key];
      //}
        ...
        if (groupedData != null) {
          ...
          if (uniqueKeys != null) {
            ...
            uniqueKeys.forEach(key => {
              ...
              total = groupedData[key].reduce((n, {
                  transactionAmount
                }) =>
              ...
              finalOutput.push({
                Id : ++id,
                Action: key.replace("misc", "other").replace("drinks", "drink"),
                TotalSpent: formatter.format(total) //as instructed in requirements
              })
            });
...
processDatasets(dataSet1, dataSet2);